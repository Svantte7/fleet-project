...
export default function LoginScreen({ navigate }) {
  ...
  const login = async () => {
    if (!name.trim() || !pin.trim()) { setErr('Syötä nimi ja PIN.'); return; }
    setBusy(true); setErr('');
    try {
      ...
      if (!profile || !profile.active) {
        setErr('Käyttäjätili ei ole aktiivinen.'); setBusy(false); return;
      }
      if (profile.mustChangePIN) {
        navigate('changePin', { userId: cred.user.uid, forced: true });
      } else {
        navigate(profile.role === 'admin' ? 'adminHome' : 'driverHome', { userId: cred.user.uid });
      }
    } catch (e) {
      if (e.code === 'auth/invalid-credential' || e.code === 'auth/wrong-password' || e.code === 'auth/user-not-found') {
        setErr('Virheellinen nimi tai PIN-koodi.');
      } else {
        setErr('Kirjautuminen epäonnistui. Yritä uudelleen.');
      }
      setBusy(false);
    }
  };
