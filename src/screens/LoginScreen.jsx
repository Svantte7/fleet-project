  ...
  const login = async () => {
    ...
    try {
      ...
      const res  = await fetch(
        `https://firestore.googleapis.com/v1/projects/kalustohallinta/databases/kalustohallinta/documents/users/${uid}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      console.log("REST status:", res.status);
      const json = await res.json();
      console.log("REST response:", JSON.stringify(json));

      const role  = json.fields?.role?.stringValue || "driver";
      const uname = json.fields?.name?.stringValue || name;

      navigate(role === "admin" ? "adminHome" : "driverHome", { userId: uid, userName: uname });
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, ${C.navy} 0%, #243B55 60%, #1a3a5c 100%)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 22 }}>
      ...
      <div style={{ width: "100%", maxWidth: 360, background: "rgba(255,255,255,0.06)", borderRadius: 22, padding: "26px 22px", border: "1px solid rgba(255,255,255,0.1)" }}>
        ...
        <button onClick={login} disabled={busy} style={{ width: "100%", background: busy ? C.muted : C.orange, color: "#fff", border: "none", borderRadius: 13, padding: "14px", fontSize: 15, fontWeight: 800, cursor: busy ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
