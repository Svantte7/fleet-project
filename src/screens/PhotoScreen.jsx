...
export default function PhotoScreen({ navigate, params }) {
  ...
  const save = () => {
    ...
    setTimeout(() => {
      DB.saveInspection({
        trailerReg, truckReg, userId,
        userName: user.name,
        photos: SIDES.filter(s => photos[s.key]).map(s => ({
          side: s.key, sideLabel: s.label,
          uri: photos[s.key].uri, takenAt: photos[s.key].takenAt,
        })),
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      <AppHeader title="Kuvaus" subtitle={trailerReg} onBack={() => navigate('regInput', { userId, truckReg, trailerReg })} />
      ...
      <div style={{ padding: '14px 16px', maxWidth: 600, margin: '0 auto' }}>
          ...
          return (
            <Card key={side.key} style={{ padding: 0, overflow: 'hidden', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                {/* Thumbnail */}
                <div
                  style={{ width: 90, height: 90, flexShrink: 0, background: ph ? 'transparent' : `${C.navy}0A`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', cursor: ph ? 'pointer' : 'default' }}
                ...
                <div style={{ paddingRight: 14 }}>
                  <Btn onClick={() => setCamSlot(side.key)} variant={ph ? 'ghost' : 'primary'} sm>{ph ? '🔄' : '📷'}</Btn>
                </div>
              </div>
            </Card>
          );
        ...
        <Card style={{ border: `2px solid ${dmgPhotos.length > 0 ? `${C.danger}44` : C.border}`, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 12 }}>
              {dmgPhotos.map((ph, i) => (
                <div key={i} style={{ position: 'relative', width: 88, height: 88, borderRadius: 9, overflow: 'hidden', cursor: 'pointer' }} onClick={() => setLightboxImg(ph.uri)}>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); setDmgPhotos(p => p.filter((_, j) => j !== i)); }}
                    style={{ position: 'absolute', top: 3, right: 3, background: 'rgba(217,79,79,0.92)', border: 'none', color: '#fff', borderRadius: 7, width: 22, height: 22, cursor: 'pointer', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >✕</button>
                </div>
              ))}
            </div>
          ...
          <Btn onClick={() => setCamSlot('damage')} variant="danger" full sm>📷  Lisää kuva vauriosta</Btn>
